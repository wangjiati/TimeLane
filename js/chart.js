class TimelineChart {
  ...
  _effectiveTextReserveHeight() {
    const pos = this.config.blockTextPosition;
  ...
  _rowHeight() {
    const c = this.config;
    return c.headerHeight + this._effectiveTextReserveHeight() + c.channelHeight + c.footerHeight + c.channelGap;
  }
  ...
  _drawChartTitle(w) {
    ...
    ctx.font = `bold ${this.config.timeFontSize + 2}px ${this.config.fontFamily}`;
  ...
  _drawLabels(w) {
    ...
    for (let i = 0; i < this._visData.length; i++) {
      ...
      if (item.type === 'group-header') {
        ...
        ctx.font = `${tFont - 1}px ${cfg.fontFamily}`;
        ctx.textBaseline = 'middle';
      ...
      if (item.type === 'channel') {
        ...
        const blockAreaTop = item.y + cfg.headerHeight + effReserve;

        ctx.fillStyle = t.headerDescColor;
        ctx.font = `${tFont - 2}px ${cfg.fontFamily}`;
        ctx.fillText(this._truncateText(ch.description || '', labelW - 16), 12, blockAreaTop - 24);

        ctx.fillStyle = t.headerTitleColor;
        ctx.font = `bold ${tFont + 1}px ${cfg.fontFamily}`;
        ctx.fillText(this._truncateText(ch.title, labelW - 16), 12, blockAreaTop - 6);
      }
    }
  }
  ...
  _drawAllChannels(w) {
    ...
    for (let i = 0; i < this._visData.length; i++) {
      ...
      if (item.type === 'channel') {
        ...
        ctx.fillRect(labelW, blockAreaTop, w - labelW, cfg.channelHeight);
        chCount++;
  ...
  _drawChannelBlocks(channel, baseline, areaTop, areaHeight) {
    ...
    for (const block of channel.blocks || []) {
      ...
      if (blockShow && block.text && bw > 30) {
        const fontSize = tc.size || cfg.blockFontSize;
  ...
  _drawChannelEvents(channel, baseline, blockAreaTop) {
    ...
    for (const ev of channel.events || []) {
      ...
      const markerSize = 4;
      const triTipY = textY + 2;
      const triBaseY = triTipY + markerSize + 3;
      ctx.beginPath();
  ...
  _drawHoverHighlight() {
    ...
    const cfg = this.config;
    const areaTop = item.y + cfg.headerHeight + this._effectiveTextReserveHeight();
  ...
  _buildSVG() {
    ...
    if (cfg.chartTitle) {
      svg += `<text x="${svgW / 2}" y="${cfg.chartTitleHeight / 2}" text-anchor="middle" dominant-baseline="central" fill="${t.headerTitleColor}" font-size="${tFont + 2}" font-weight="bold">${this._escapeXml(cfg.chartTitle)}</text>\n`;
    }
    ...
    for (const item of this._visData) {
      if (item.type === 'group-header') {
        ...
        svg += `<text x="${labelW + 12}" y="${item.y + cfg.groupHeaderHeight / 2}" text-anchor="start" dominant-baseline="central" fill="${t.groupHeaderTextColor}" font-size="${tFont - 1}" font-weight="bold">${this._escapeXml(item.title)}</text>\n`;
        continue;
      }
      ...
      if (item.type === 'channel') {
        ...
        svg += `<rect x="${labelW}" y="${blockAreaTop}" width="${svgW - labelW}" height="${areaHeight}" fill="${bgColor}" />\n`;
        svgChCount++;

        svg += `<text x="12" y="${blockAreaTop - 24}" text-anchor="start" dominant-baseline="baseline" fill="${t.headerDescColor}" font-size="${tFont - 2}">${this._escapeXml(ch.description || '')}</text>\n`;
        svg += `<text x="12" y="${blockAreaTop - 6}" text-anchor="start" dominant-baseline="baseline" fill="${t.headerTitleColor}" font-size="${tFont + 1}" font-weight="bold">${this._escapeXml(ch.title)}</text>\n`;
        ...
        for (const block of ch.blocks || []) {
          ...
          const blockShow = tc.show !== false && cfg.blockTextShow !== false;
          if (blockShow && block.text && bw > 30) {
            const fs = tc.size || cfg.blockFontSize;
            ...
            switch (pos) {
              ...
              case 'above':
                if (bh > areaHeight * 0.7) {
                  textY = by + 3 + fs; anchor = 'middle'; dv = 'hanging';
                } else {
                  textY = blockAreaTop - effReserve + fs; anchor = 'middle'; dv = 'baseline';
                }
                break;
              default:
        ...
        if (cfg.showEvents) {
          for (const ev of ch.events || []) {
            ...
            const triTipY = evTextY + 2;
            const triBaseY = triTipY + ms + 3;
